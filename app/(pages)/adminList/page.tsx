...
export default async function AdminListPage({ searchParams }: { searchParams?: { itemsPerPage?: string, currentPage?: string } }) {

    const itemsPerPage = Number(searchParams?.itemsPerPage) || 10;
    const currentPage = Number(searchParams?.currentPage) || 1;
    const totalPage = await readAdminTotalPage(itemsPerPage);
    ...
    return (
        <>
            ...
            <Suspense fallback={<TableSkeleton columnCount={2} rowCount={10} />}>
                <DataTable itemsPerPage={itemsPerPage} currentPage={currentPage} readAction={readAction} columnListDisplay={columnListDisplay} primaryKey={primaryKey} />
            </Suspense>
